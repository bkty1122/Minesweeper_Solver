# https://github.com/JohnnyDeuss/minesweeper-solver/blob/master/minesweeper_solver/solver.py
...
class Solver:
    def __init__(self, width, height, total_mines, stop_on_solution=True):
        ...
        # init 中，全格都是 np.nan 代表，因為未有已知的空格,np.nan 是float type 
    ...
    def solve(self, state):
        ...
        state = np.array([[state[y][x] if isinstance(state[y][x], int) 
                           else np.nan for x in range(len(state[0]))] for y in range(len(state))])
        # Are there any opened squares; is this the first move?
        if not np.isnan(state).all():
        # 如果不是全部state 都是 np.nan
            # Expand the known state with new information from the passed state.
            self.known[~np.isnan(state)] = 0
            # ~ 代表把數值二進制反轉，如a = 5, ~a = -6, 這邊想檢查哪個element in state 不是 nan 的 (用 inf 也可替代)
            # but why the reversed nan = 0?
    ...
    def _combine_components(self, state, prob, c_probs, c_counts):
        """ Combine the probabilities and model counts found in the CP step into one probability array.
            ...
            :param c_counts: A list of model count mappings per component, each having the format {num_mines: model count}
            :returns: The exact probability for every unknown square.
        """
        ...
        # & is "and" in 'bitwise' mode. while in bitwise mode, data would combine in 2-bit mode. 
        # in python, while running &, choosing the 'lowest 2-bit' if those set of data are different
        unconstrained_squares = np.isnan(state) & ~solution_mask & np.isnan(self.known)
        # by using solution_mask and unconstratined_squares operater, we can find squares that are inside the guessing area
        n = unconstrained_squares.sum(dtype=int)
        # n = number of mines that are nan and not included in the guessing areas. use to calculate
        # the overall probabilty of cells in other areas
        ...
        if c_probs:
            ...
            # for c_ms in product(*[d for d in c_probs]): ?
                m = sum(c_ms)
                #為什麼要用itertools gen key combinations in c_prob?, as well as their sum into m? 
                #為什麼要比較 mine_left - n (number of mines in the area) vs m vs mines_left? 
                if self.mines_left() - n <= m <= mines_left:
                    ...
                    #把 comb_prob reduce to a single probability
    ...
    def _components(self, state):
        ...
        # numbers_musk return only True and False array, True means squares in states that has been opened
        ...
        # from label module, it would output a labeled array, and the number of components in the form of np.array
